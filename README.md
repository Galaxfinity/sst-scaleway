...
- **Removal policy is respected.** With `removal: "retain"`, the data-carrying resources (`Bucket`, `Postgres`) are retained on removal - the same rule SST applies to its aws data resources, registered here for the Scaleway types. `removal: "retain-all"` is enforced by SST itself for every resource. With `removal: "remove"`, buckets force-destroy their objects so removal actually succeeds.
